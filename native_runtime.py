...
POINTS = {'set_text': 0x588A40, 'update': 0x584CA0, 'destroy': 0x5839F0,
          'ruby_context_init': 0x5830F0, 'ruby_place_return': 0x58714F,
          'text_lookup':0x5e8550,'label_text_key_load':0x584842,
          'node_name_lookup':0x5814d8,'ruby_measure_return':0x587084,
          'newline_prepare':0x587966,
          'ruby_base_measure_end':0x586e9c,'ruby_compensate':0x587253,
          'ruby_end':0x587349,'parse_text':0x5877a0,
          'layout_create':0x58fc80,'layout_release':0x58ff50,
          'dialogue_popup':0x4ae2a0,'dialogue_message':0x4ae990,
          'dialogue_bubble':0x4aee20,'dialogue_builder':0x4ad670}
...
def native_report(exe):
    ...
    try:
        report['native'] = {name: {'rva': rva, 'bytes': pe.get_data(rva, 16).hex()}
...
def exact_dictionary(entries, primary, secondary, mode='bilingual'):
    ...
    for entry in entries:
        texts = entry['texts']
        pair = (texts[primary], texts[secondary]) if primary in texts and secondary in texts else None
...
class NativeLabels:
    ...
    def attach(self, pid, exe, dictionary=None, *, model=None, config=None, mode='annotation'):
        ...
        source = '\n'.join((ROOT/name).read_text(encoding='utf-8') for name in
                           ('runtime_text.js','runtime_identity.js','native_hash.js','native_agent.js','native_transport.js'))
        self.session = frida.attach(pid)
        def detached(reason,*args):
            try:self.callback({'type':'detached','reason':reason})
        ...
        try:
            ...
            if report.get('text_table_global'):
                probe=self.session.create_script('''rpc.exports={ready(){try{
                  const base=Process.getModuleByName('sora_2nd.exe').base;
                  const manager=base.add('''+str(report['text_table_global'])+''').readPointer();
                ...
                while not probe.exports_sync.ready():
                    if self.exited.wait(.25):raise RuntimeError('游戏在初始化期间退出')
                probe.unload()
            self.script = self.session.create_script('const REPORT=' + json.dumps(report) + ';\n' + source)
            startup_errors = []
            def on_message(message, _):
                value = message.get('payload', {'type': 'error', 'message': message.get('description', str(message))})
            ...
            else:self.script.exports_sync.configure(dictionary or {}, False, 1)
        except Exception:
    ...
    def load(self,model,config,mode):
        rpc=self.script.exports_sync;token=uuid.uuid4().hex
        ...
        try:
            ...
            size=256*1024;count=0
            for at in range(0,len(text),size):
                rpc.modelpart(token,count,text[at:at+size]);count+=1
            return rpc.modelcommit(token,count,len(text),mode,bool(config['enabled']),config.get('annotation_scale',.9),
                {k:config[k] for k in ('ruby_scale','ruby_gap','ruby_offset_x','line_gap') if k in config})
        except Exception:
            try:rpc.modelabort(token)
            except Exception:pass
            raise
    ...
    def style(self,config):
        return self.script.exports_sync.style(config.get('annotation_scale',.9),
            {k:config[k] for k in ('ruby_scale','ruby_gap','ruby_offset_x','line_gap') if k in config})
