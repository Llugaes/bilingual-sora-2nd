...
- 注解资格比较折叠全角 ASCII 并忽略显示控制，纯图标和等价数字不新增标记。字号缩小只作用于有注解的正文；混合未翻译文字的标签保留原字号及文字推进，新增注解不触发引擎对后续正文的基线补偿。普通与独立注解使用同一个原生测量范围和正文实际字形上缘计算垂直间距；保留原注音时新增层位于其上方，原层不动。间距以本地排版坐标计量，屏幕缩放和字形留白仍可能影响视觉间隔。
...
原生 SetText 后仍会发生字号赋值；每个已追踪标签记录生成注解时的字号，更新回调发现字号变化才重排。没有变化时不重复调用 SetText。独立注解利用已有的正文测量调用得到实际字形上缘，测量不绘制且清除空锚点的尺寸贡献；正文、颜色、原注音不被移动。组合文本按原换行和空白分段，完整匹配优先，所有可配对行均被处理。
