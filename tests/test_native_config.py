...
from native_config import ActionPolicy,BackendLock,read_config,write_config,normalize_config
from native_runtime import NativeLabels
from native_probe import write_telemetry

def states(press=None,hold=False):
    return {a:SimpleNamespace(pressed=a==press,held=hold if a=='language_hold' else False)
...
class NativeConfigTests(unittest.TestCase):
    def test_malformed_config_is_rejected_as_recoverable_validation_error(self):
        cases=[{'primary':[]},{'hotkeys':None},{'interaction':{}},{'enabled':'false'},
            {'scope':[]},{'sources':None},{'annotation_scale':True},{'overlay_binding':{'keyboard':None}},
            {'overlay_binding':{'keyboard':['UNKNOWN_KEY']}},
            {'overlay_binding':{'gamepad':{'buttons':[[]]}}},
            {'overlay_binding':{'gamepad':{'axes':[None]}}},
            {'overlay_binding':{'gamepad':{'axes':[{'index':0,'direction':1,'threshold':float('nan')}]}}}]
        for value in cases:
    ...
    def test_live_backend_is_never_detached_even_with_no_modified_labels(self):
        ...
        native.exited.set();self.assertTrue(native.detach_if_restored())

    def test_os_lock_prevents_second_backend_and_can_be_reacquired(self):
        with tempfile.TemporaryDirectory() as tmp:
            path=Path(tmp)/'backend.lock';one=BackendLock(path)
            try:
                with self.assertRaises((RuntimeError,OSError)):BackendLock(path)
            finally:one.close()
            BackendLock(path).close()

    def test_transient_windows_reader_does_not_lose_atomic_status_update(self):
        with tempfile.TemporaryDirectory() as tmp:
            path=Path(tmp)/'status.json';write_config({'old':True},path)
            replace=os.replace;attempts=[]
            def transient(source,destination):
                attempts.append(1)
                if len(attempts)<3:raise PermissionError('reader still open')
                replace(source,destination)
            with patch('native_config.os.replace',side_effect=transient),patch('native_config.time.sleep'):
                self.assertTrue(write_telemetry({'new':True},path))
            self.assertEqual(read_config(path)['new'],True);self.assertEqual(len(attempts),3)
            self.assertEqual(list(Path(tmp).glob('*.tmp')),[])

    def test_persistent_status_lock_never_escapes_into_resident_backend_shutdown(self):
        with tempfile.TemporaryDirectory() as tmp:
            path=Path(tmp)/'status.json';write_config({'old':True},path)
            with patch('native_config.os.replace',side_effect=PermissionError('locked')),patch('native_config.time.sleep') as pause:
                self.assertFalse(write_telemetry({'new':True},path))
                self.assertLess(sum(call.args[0] for call in pause.call_args_list),.01)
            self.assertTrue(read_config(path)['old']);self.assertEqual(list(Path(tmp).glob('*.tmp')),[])
