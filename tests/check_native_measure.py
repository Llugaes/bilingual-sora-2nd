...
PARSER_SOURCE = (ROOT / "sora_bilingual/game/scripts/native_parser.js").read_text("utf-8")
SOURCE = (
    PARSER_SOURCE
    + "\n"
    + (ROOT / "sora_bilingual/game/scripts/native_measure.js").read_text("utf-8")
)
...
`,{});
const parserContexts=new Map(),parserErrors=[];
const scaleTracker=createNativeParser(parserContexts,error=>parserErrors.push(String(error)));
const parserFixture=new CModule('int parser_cleanup(unsigned char *label,unsigned char *parser) { return parser[0x1a5]+1; }');
const parserCleanup=new NativeFunction(parserFixture.parser_cleanup,'int',['pointer','pointer']);
...
rpc.exports={run(){
 ...
 const noTrackerErrors=[],noTracker=makeMeasureWithoutTracker(noTrackerErrors);check(noTracker.status().bridgeScopes===0&&noTrackerErrors.length===0,'optional scale tracker CModule path did not compile');
 ...
 const integratedErrors=[],integrated=makeMeasure(integratedErrors,{measurement:measuredCaller,baseMeasurement:baseCaller}),integratedScope=labelAndParser(),integratedTarget=target(),thread=Process.getCurrentThreadId();prepareLabel.writePointer(integratedScope.label);prepareParser.writePointer(integratedScope.parser);prepareRestored.writeU64(0);const integratedToken=integrated.push(thread,integratedScope.label,integratedScope.owned,.375);check(tokenOK(integratedToken),'integrated scope push');const prepareHook=Interceptor.attach(fixture.ruby_context_init_fixture,{onEnter:prepare.prepare_on_enter,onLeave:prepare.prepare_on_leave}),integratedHook=Interceptor.attach(fixture.ruby_context_init_fixture,{onEnter:integrated.onEnter,onLeave:integrated.onLeave});Interceptor.flush();check(runMeasure(integratedTarget,1).toNumber()===132,'integrated fast return');let integratedStatus=integrated.status();check(integratedStatus.fastMeasure===1&&integratedStatus.slow===0,'integrated fast measure path');check(fbits(integratedTarget.add(0x158))===productBits(1.234567,.375)&&fbits(integratedTarget.add(0x15c))===productBits(7.654321,.375),'integrated fast float bits');const inheritedScale=integratedTarget.add(0x15c).readFloat();integratedTarget.add(0x158).writeFloat(1.5);integratedTarget.add(0x15c).writeFloat(1.5);check(scaleTracker.applySize(integratedTarget),'fast path did not retain ruby scale for S5');check(fbits(integratedTarget.add(0x158))===productBits(1.5,inheritedScale)&&fbits(integratedTarget.add(0x15c))===productBits(1.5,inheritedScale),'fast S5 scale differs from native init result');const cleanupHook=Interceptor.attach(parserFixture.parser_cleanup,{onEnter:scaleTracker.onEnter,onLeave:scaleTracker.onLeave});Interceptor.flush();check(parserCleanup(Memory.alloc(0x500),integratedTarget)===1,'tracked parser cleanup return');cleanupHook.detach();Interceptor.flush();check(scaleTracker.status().active===0,'fast tracked parser state leaked');const baseTarget=target();check(runBase(baseTarget,1).toNumber()===132,'integrated base return');integratedStatus=integrated.status();check(integratedStatus.fastBase===1&&integratedStatus.slow===0,'integrated fast base path');check(fbits(baseTarget.add(0x158))===productBits(1.234567,1)&&fbits(baseTarget.add(0x15c))===productBits(7.654321,1),'integrated base must not scale');check(!scaleTracker.applySize(baseTarget),'base branch must not retain a size factor');check(prepareRestored.readU64().toNumber()===2,'prepare listener did not restore register context');integratedHook.detach();prepareHook.detach();Interceptor.flush();check(integrated.pop(thread,integratedToken),'integrated scope finally pop');integratedStatus=integrated.status();check(integratedStatus.pops===1,'integrated scope cleanup');check(integratedErrors.length===0,'integrated errors '+integratedErrors);
 ...
 const overflowParserErrors=[],overflowTracker=createNativeParser(new Map(),error=>overflowParserErrors.push(String(error))),overflowMeasureErrors=[],overflowMeasure=makeMeasureWithTracker(overflowTracker,overflowMeasureErrors,{measurement:measuredCaller,baseMeasurement:baseCaller}),overflowScaleSlots=[];for(let i=0;i<1025;i++){const p=Memory.alloc(0x200);overflowScaleSlots.push(p);overflowTracker.trackScale(p,.4);}check(overflowTracker.status().fallback&&overflowTracker.status().overflows===1,'overflow tracker setup');const overflowFastScope=labelAndParser(),overflowTarget=target(),overflowToken=overflowMeasure.push(thread,overflowFastScope.label,overflowFastScope.owned,.375);check(tokenOK(overflowToken),'overflow fast scope push');prepareLabel.writePointer(overflowFastScope.label);prepareParser.writePointer(overflowFastScope.parser);const overflowPrepare=Interceptor.attach(fixture.ruby_context_init_fixture,{onEnter:prepare.prepare_on_enter,onLeave:prepare.prepare_on_leave}),overflowHook=Interceptor.attach(fixture.ruby_context_init_fixture,{onEnter:overflowMeasure.onEnter,onLeave:overflowMeasure.onLeave});Interceptor.flush();check(runMeasure(overflowTarget,1).toNumber()===132,'overflow fast return');const overflowInherited=overflowTarget.add(0x15c).readFloat();overflowTarget.add(0x158).writeFloat(1.5);overflowTarget.add(0x15c).writeFloat(1.5);check(overflowTracker.applySize(overflowTarget)&&fbits(overflowTarget.add(0x15c))===productBits(1.5,overflowInherited),'overflow fast measure scale bridge lost S5 factor');const overflowCleanup=Interceptor.attach(parserFixture.parser_cleanup,{onEnter:overflowTracker.onEnter,onLeave:overflowTracker.onLeave});Interceptor.flush();check(parserCleanup(Memory.alloc(0x500),overflowTarget)===1,'overflow tracked parser cleanup return');for(const p of overflowScaleSlots)check(parserCleanup(Memory.alloc(0x500),p)===1,'overflow original slot cleanup');overflowCleanup.detach();overflowHook.detach();overflowPrepare.detach();Interceptor.flush();check(overflowMeasure.pop(thread,overflowToken),'overflow fast scope pop');check(overflowMeasure.status().fastMeasure===1&&overflowMeasure.status().slow===0&&overflowMeasureErrors.length===0&&overflowParserErrors.length===0&&overflowTracker.status().active===0,'overflow fast bridge fallback');
 ...
 check(errors.length===0,errors.join('; '));check(parserErrors.length===0,parserErrors.join('; '));
 ...
 report.validation={optional_scale_tracker:false,fast_status:status,fast_predicate:{measurement:1,base:2,owned_pointer_change:0,measuring_false:0,wrong_caller:0},float_bits:{x:productBits(1.234567,.375),y:productBits(7.654321,.375)},args_and_return:EXPECTED,direct_js:{enters:js.enters,leaves:js.leaves},slow_bridge:{slow:status.slow,scopes:status.bridgeScopes},integrated_fast:{measured_caller:String(measuredCaller),base_caller:String(baseCaller),status:integratedStatus,prepare_restores:2,size_tracker:scaleTracker.status()},integrated_guards:{wrong_label:'slow legacy callback',changed_owned_pointer:'slow legacy callback',parser_measuring_flag:false,status:guardStatus,prepare_restores:3},overflow_fast_measure:{status:overflowMeasure.status(),size_tracker:overflowTracker.status()},nested_and_thread_scope:'controlled thread IDs 11 and 22 plus nested stack',fail_closed:{nonfinite:true,mismatched_pop:true,scope_overflow:true}};
