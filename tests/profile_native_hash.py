...
def main():
    ...
    try:
        ...
        source=(ROOT/'runtime_identity.js').read_text('utf-8')
        if args.native:source+='\n'+(ROOT/'native_hash.js').read_text('utf-8')
...
'''
        agent=session.create_script(source);agent.load()
        result=[]
        for data in samples:
            times=[]
            for _ in range(5):
                start=time.perf_counter();digest=agent.exports_sync.bench(data);times.append((time.perf_counter()-start)*1000)
                assert digest==hashlib.sha256(data).hexdigest()
            result.append({'bytes':len(data),'min_ms':min(times),'median_ms':sorted(times)[2],'max_ms':max(times)})
            if args.native:
        ...
        output={'backend':'windows-cng' if args.native else 'pure-js','frida_default_runtime':True,
                'includes_rpc_transport':True,'samples':result,'native_pointer_identity_verified':args.native,
                'game_started':False,'game_attached':False}
        path=ROOT/'generated'/('hash-performance-after.json' if args.native else 'hash-performance-before.json')
        path.write_text(json.dumps(output,indent=2),'utf-8');print(json.dumps(output,indent=2))
    finally:
