...
class MenuTextTests(unittest.TestCase):
    def test_incomplete_or_blank_locale_never_erases_source_or_invents_translation(self):
        for missing in (None,'','   '):
            ...
            if missing is not None:texts['es']=missing
    ...
    def test_composed_description_keeps_icons_colour_and_values(self):
        tr=MenuTranslator([entry('强化','強化'),entry('单体','単体'),
            entry('HP上限+%d','最大HP+%d'),entry('说明。','説明。')],'zh-Hans','ja')
        raw='强化【<I299>单体：<c698>HP上限+20</C>】\n说明。'
        result=tr.translate(raw)
        self.assertEqual(result,'<R>强化</R強化>【<I299><R>单体</R単体>：<c698><R>HP上限+20</R最大HP+20></C>】\n<R>说明。</R説明。>')
        self.assertEqual(tr.translate(raw,'secondary'),'強化【<I299>単体：<c698>最大HP+20</C>】\n説明。')

    def test_no_partial_word_replacement_or_ambiguous_translation(self):
        tr=MenuTranslator([entry('药','薬'),entry('保存','セーブ'),entry('保存','セーブする')],'zh-Hans','ja')
    ...
    def test_text_key_resolves_duplicate_word_without_weakening_raw_match(self):
        entries=[{'key':'table/t_text.tbl/TXT_SAVE',**entry('保存','セーブ')},
    ...
    def test_string_template_translates_known_name_and_keeps_unknown_value(self):
        tr=MenuTranslator([entry('选择%s？','%sを選択？'),entry('艾丝蒂尔','エステル')],'zh-Hans','ja')
    ...
    def test_exact_description_suffix_disambiguates_item_effect_header(self):
        entries=[{'key':'table/t_itemhelp.tbl/type/label',**entry('强化','強化')},
        ...
        self.assertEqual(tr.translate('强化\n这是药。'),'<R>强化</R強化>\n<R>这是药。</Rこれは薬。>')
