...
def main():
    ...
    navi = [
        e
        for e in entries
        if e["key"].startswith("table/t_quest.tbl/NaviText/") and e["key"].endswith("/title")
    ]
    ...
    report = {
        "game_started": False,
        "game_attached": False,
        "navigation_objectives": len(navi),
        "models": [],
    }
...
"""
    try:
        ...
        for primary in ("zh-Hans", "en"):
            ...
            run = subprocess.run(
                ["node", "-e", runner, str(path), str(cases_path), primary],
                cwd=ROOT,
                text=True,
                capture_output=True,
            )
    ...
    (ROOT / "generated/save-localization-check.json").write_text(
