...
from native_runtime import exact_dictionary
...
class NativeDictionaryTests(unittest.TestCase):
    def test_auto_attach_waits_for_text_manager_before_installing_hooks(self):
        ...
        def readiness():calls.append('ready');return next(ready)
        probe=SimpleNamespace(load=lambda:calls.append('probe_load'),unload=lambda:calls.append('probe_unload'),
            exports_sync=SimpleNamespace(ready=readiness))
        agent=SimpleNamespace(on=lambda *_:None,load=lambda:calls.append('agent_load'),
        ...
        with patch('native_runtime.native_report',return_value={'text_table_global':1}),\
             patch('native_runtime.frida.attach',return_value=session):
            NativeLabels(lambda _:None).attach(42,'unused')
        self.assertEqual(calls,['probe_load','ready','ready','ready','probe_unload','agent_load','configure'])
    ...
    def test_missing_language_cannot_borrow_another_keys_pair(self):
        entries=[{'texts':{'ja':'a','zh-Hans':'一'}},{'texts':{'en':'one','zh-Hans':'一'}}]
        self.assertNotIn('一',exact_dictionary(entries,'zh-Hans','ja'))
    ...
    def test_annotation_rejects_nested_tags_and_newlines(self):
        entries=[{'texts':{'ja':'ティアの薬','zh-Hans':'回复药'}},
