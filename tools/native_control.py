...
if args.captured_description:
    rows = [
        ...
        for line in (root / "probe-ui.jsonl").read_text(encoding="utf-8").splitlines()
    ]
    matches = [
        r["text"] for r in rows if r.get("kind") == "setter" and r.get("caller") == "0x137e08"
    ]
    if not matches:
...
config.update(
    enabled=args.action
    in ("secondary", "bilingual", "annotation", "language-toggle", "language-hold"),
    replay=args.action == "replay",
    stop=args.action == "stop",
    mode=args.action if args.action in ("secondary", "annotation") else "bilingual",
)
config["interaction"] = (
    args.action.replace("-", "_")
    if args.action in ("annotation", "language-toggle", "language-hold")
    else None
)
...
print(
    args.action,
    "scope:",
    config.get("scope", "selected"),
    "selected source strings:",
