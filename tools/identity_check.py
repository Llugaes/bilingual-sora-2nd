...
def main():
        ...
        for sig, bucket in model["script_identities"]["scripts"].items():
            for script in bucket:
                scripts.append(
                    {
                        "signature": sig,
                        "data": base64.b64encode(archive.read(paths[script["paths"][0]])).decode(),
                        "sha256": script["sha256"],
                        "functionName": next(iter(script["functions"])),
                    }
                )
                for path in script["paths"]:
                    for fn, context in script["functions"].items():
                        contexts[path + "/" + fn] = (sig, script["sha256"], context)
                for fn, context in script["functions"].items():
                    for token, call in context["calls"].items():
                        ...
                        identity = {
                            ...
                            "sha256": script["sha256"],
                            "functionName": fn,
                            "argumentsToken": actual,
                        }
                        for source, (a, b) in call["model"]["pairs"].items():
                            if not source.strip():
                                continue
                            tr = MenuTranslator(
                                [{"texts": {"s": source, "a": a, "b": b}}], "a", "b", "s", True
                            )
                            cases.append(
                                {
                                    "identity": identity,
        ...
        for key in gap.get("keys", []):
            ...
            if key.startswith("table/"):
                ...
                if direct and same_pair(direct["model"]["pairs"].get(source), pair):
                    ok = True
                    counts["script_records_with_pointer_route"] += 1
                path, rest = key.split(".dat/", 1)
                parts = rest.split("/")
                found = contexts.get(path + ".dat/" + parts[0])
                if found:
                    context = found[2]
                    if len(parts) > 2 and parts[1] == "called":
                        for call in context["calls"].values():
                            if int(parts[2]) not in call["records"]:
                                continue
                            if same_pair(call["model"]["pairs"].get(source), pair):
                                ok = True
                                break
                            # A fragment can be ambiguous inside a call, while
                            # its complete block retains the exact localised text.
                            alignment = (
                                "/alignment/" + key.split("/alignment/", 1)[1]
                                if "/alignment/" in key
                                else ""
                            )
                            whole = by_key.get(
                                key.split("/arg/")[0] + "/assembled_dialogue" + alignment
                            )
                            if whole and whole["texts"].get("zh-Hans") in call["model"]["pairs"]:
                                wt = whole["texts"]
                                expected = [wt.get("zh-Hans"), wt.get("ja")]
                                if (
                                    same_pair(call["model"]["pairs"][expected[0]], expected)
                                    and source in expected[0]
                                    and t["ja"] in expected[1]
                                ):
                                    ok = True
                                    counts["fragment_records_covered_by_complete_call"] += 1
                                    break
            ...
            if ok:
                ...
                bad.append(key)
        counts["records_resolved"] += good
        counts["records_remaining"] += len(bad)
        counts[
            "sources_all_records_resolved"
            if not bad
            else "sources_partial"
            if good
            else "sources_remaining"
        ] += 1
        if bad:
            remaining.append({"source": gap["source"], "keys": bad, "resolved_keys": good})
    table_files = {}
    import hashlib
    ...
    with FpacArchive(GAME / "pac/steam" / _TABLE_ARCHIVES["zh-Hans"]) as archive:
        for path, actual in _logical_tables(archive).items():
            ...
            if digest in model["table_identities"]["files"]:
                table_files[digest] = base64.b64encode(data).decode()
    ...
    try:
        ...
        proc = subprocess.run(
            ["node", str(ROOT / "tests/replay_identity.js"), str(fixture)],
            capture_output=True,
            text=True,
            encoding="utf-8",
            check=True,
    ...
    result = {
        ...
        "game_started": False,
        "game_attached": False,
        "meaning": "Offline record routing and content preservation; native reachability and visuals require game acceptance.",
    }
    (ROOT / "generated/identity-review.json").write_text(
        json.dumps(result, ensure_ascii=False, indent=2), "utf-8"
    )
    print(
        json.dumps(
            {k: v for k, v in result.items() if k != "remaining"}, ensure_ascii=False, indent=2
        )
    )
