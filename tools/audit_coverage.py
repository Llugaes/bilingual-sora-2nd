...
from sora_bilingual.localization.menu_text import MenuTranslator, visual_secondary
...
def audit(entries, primary="zh-Hans", secondary="ja", source_language="zh-Hans"):
    ...
    for source, (a, b) in tr.pairs.items():
        ...
        if plan["kind"] == "layered":
            ...
            if "<R>" in a + b:
                counts["original_ruby_preserved"] += 1
    ...
    for e in entries:
        ...
        if primary not in t or secondary not in t:
            counts["entries_missing_locale"] += 1
            continue
        if not needs_annotation(t[primary], t[secondary]):
            continue
    ...
    for source, keys in candidates.items():
        ...
        if source in tr.plain_pairs and plan["kind"] in ("ruby", "layered"):
            counts["normalized_or_composite_sources"] += 1
            continue
        if plan["kind"] in ("ruby", "layered"):
            counts["partially_annotated_ambiguous_sources"] += 1
        if any(s == source for _, s, _ in tr.keyed):
            counts["requires_text_key"] += 1
            continue
        if any(s.render(source)["kind"] in ("ruby", "layered") for s in tr.scoped.values()):
            counts["requires_owner_scope"] += 1
            continue
        ...
        gaps.append(
            {"reason": "ambiguous_source_requires_runtime_identity", "source": source, "keys": keys}
        )
...
def main():
    ...
    (ROOT / "generated/coverage-review.json").write_text(
        json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    print(
        json.dumps(
            {k: v for k, v in result.items() if k not in ("gaps", "cases")},
            ensure_ascii=False,
            indent=2,
        )
    )
