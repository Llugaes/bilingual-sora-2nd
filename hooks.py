...
HOOKS = [
    ...
    ('blackJackDialogue1', 'e8 ?? ?? ?? ?? ?? 8d 8d 80 07 00 00 ba 08 00 00 00 66 90 0f 10 00 0f', 0),
...
def signature_matches(data: bytes, pattern: str):
    regex = b''.join(b'.' if v == '??' else re.escape(bytes.fromhex(v))
...
def script_source(report):
    ...
    return '''
'use strict';
const hooks = ''' + json.dumps(report['hooks']) + ''';
