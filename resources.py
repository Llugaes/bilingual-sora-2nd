...
from locales import LANGUAGES,archive_names
...
def assembled_dialogue(call: Called) -> str | None:
    ...
    """
    if call.kind!=3 or len(call.args)<3 or call.args[0]!=('int',5) or call.args[1] not in (('int',0),('int',6),('int',7),('int',19)):
        return None
    first=next((i for i,(kind,_) in enumerate(call.args) if kind=='string'),None)
    if first is None:return None
    parts=[]
    for kind,value in call.args[first:]:
        if kind=='string':parts.append(str(value))
        elif (kind,value)==('int',10):parts.append('\n')
        else:return None
    return ''.join(parts)
...
class Function:
    ...
    def shape(self) -> tuple[object, ...]:
        return (self.flags, self.arg_types, tuple(call.shape() for call in self.called), self.code_shape)
...
def _parse_code(
    ...
    while True:
        ...
        if opcode == 0:
            ...
            if (candidate == func_id and pos + 10 <= len(data) and data[pos + 4] == 0 and data[pos + 5] == 4
            ...
            if kind == "string":
                ...
                if string_offsets is not None:string_offsets.append(candidate & 0x3fffffff)
...
def parse_scp(data: bytes) -> Script:
    ...
    magic, func_start, func_count, global_start, global_count, reserved = _SCP_HEADER.unpack_from(data)
    ...
    for number in range(func_count):
        ...
        code_at, arg_count, flags, default_count, default_at, args_at, called_count, called_at, crc, name_value = _FUNCTION.unpack_from(data, at)
    ...
    functions: dict[str, Function] = {}
    for index, (name, code_at, flags, arg_types, called_count, called_at, number) in enumerate(ordered):
        called: list[Called] = []
        ...
        code_shape, code_strings = _parse_code(data, code_at, end, number, tuple(names), tuple(globals_))
...
def _script_paths(entries_by_language: Iterable[dict[str, str]]) -> list[str]:
    return sorted({path for entries in entries_by_language for path in entries
...
def align_functions(path, function_name, functions, audit):
    ...
    for family,method in (("called","called_sequence_shape"),("code","shape")):
        groups={}
        for lang,fn in functions.items():groups.setdefault(getattr(fn,method)(),[]).append(lang)
        for group_index,(shape,languages) in enumerate(groups.items()):
            for lang in languages:
                ...
                if group_index:
                    metric="functions_called_mismatch_" if family=="called" else "functions_mismatch_"
                    _add_counter(audit,metric+lang)
            if len(languages)<2:
                _add_counter(audit,"alignment_classes_without_secondary");continue
            suffix=("/alignment/"+hashlib.sha256(repr(shape).encode()).hexdigest() if len(groups)>1 else "")
            reference=functions[languages[0]]
            def emit(key,texts):
                entries.append({'key':f'{path}/{function_name}/{key}'+suffix,'texts':texts})
                for lang in texts:_add_counter(audit,'entries_with_'+lang)
            if family=="code":
                for ordinal in range(len(reference.code_strings)):
                    emit(f'code/{ordinal}',{l:functions[l].code_strings[ordinal] for l in languages})
                continue
            for index,call in enumerate(reference.called):
                ...
                if all(t is not None for t in complete.values()):
                    ...
                    if display!=complete:emit(f'called/{index}/assembled_display',display)
                    _add_counter(audit,'assembled_dialogue_calls')
                for slot,_ in call.display_text_slots():
                    emit(f'called/{index}/arg/{slot}',{l:str(functions[l].called[index].args[slot][1]) for l in languages})
    return entries
...
def build_catalog(game_dir: str | Path, output_dir: str | Path) -> dict[str, object]:
    ...
    try:
        for language, archive_name in _ARCHIVES.items():
            ...
            if not path.is_file():
                diagnostics.append({"language":language,"reason":f"missing script archive: {path}"})
                continue
            archives[language] = FpacArchive(path)
        if not archives:raise FileNotFoundError("No script archives found")
        logical_entries = {language: _logical_script_entries(archive) for language, archive in archives.items()}
        ...
        for path in paths:
            ...
            for language in archives:
                if path not in logical_entries[language]:continue
                try:
                    parsed[language] = parse_scp(archives[language].read(logical_entries[language][path]))
            ...
            for name in names:
                functions={l:script.functions[name] for l,script in parsed.items() if name in script.functions}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build a validated multilingual Trails SCP catalogue")
