...
DEFAULT_CONFIG = {
    "primary_language": "zh-Hans", "secondary_language": "ja",
    "overlay_enabled": True, "font_size": 24, "secondary_font_size": 21,
    "bottom_offset_percent": 4, "width_percent": 80, "display_mode": "two_lines",
...
class SettingsWindow(QWidget):
    ...
    def __init__(self, config: dict[str, Any] | None = None, languages: list[Any] | None = None,
                 callbacks: dict[str, Any] | None = None) -> None:
        super().__init__()
        ...
        self.font_size = QSpinBox(); self.font_size.setRange(12, 96); self.font_size.setSuffix(" px")
        self.bottom_offset = QSpinBox(); self.bottom_offset.setRange(0, 35); self.bottom_offset.setSuffix(" %")
        self.width = QSpinBox(); self.width.setRange(25, 100); self.width.setSuffix(" %")
        self.mode = QComboBox(); self.mode.addItem("双语两行", "two_lines"); self.mode.addItem("仅副字幕", "secondary_only")
        form.addRow("主语言", self.primary); form.addRow("副语言", self.secondary)
        form.addRow(self.enabled); form.addRow("字号", self.font_size)
        form.addRow("距游戏底部", self.bottom_offset); form.addRow("字幕宽度", self.width)
        ...
        hotkey_row.addWidget(self.hotkey_label, 1); hotkey_row.addWidget(self.record_keyboard); hotkey_row.addWidget(self.record_controller)
        ...
        layout.addWidget(self.device_label); layout.addWidget(self.status_label)
        layout.addWidget(QLabel("显示方式：无边框桌面透明窗（原型，非原生游戏 UI）"))
        self.connect_button.setText("附加已有 Sora_2nd")
        layout.addWidget(self.connect_button)
        self._apply_controls()

        for widget, signal in ((self.primary, self.primary.currentIndexChanged), (self.secondary, self.secondary.currentIndexChanged),
                               (self.enabled, self.enabled.toggled), (self.font_size, self.font_size.valueChanged),
                               (self.bottom_offset, self.bottom_offset.valueChanged), (self.width, self.width.valueChanged),
    ...
    def _current_config(self) -> dict[str, Any]:
        ...
        result.update({
            "primary_language": self.primary.currentData(), "secondary_language": self.secondary.currentData(),
            "overlay_enabled": self.enabled.isChecked(), "font_size": self.font_size.value(),
            "bottom_offset_percent": self.bottom_offset.value(), "width_percent": self.width.value(),
...
class OverlayWindow(QWidget):
    def __init__(self, config: dict[str, Any] | None = None) -> None:
        flags = Qt.WindowType.FramelessWindowHint | Qt.WindowType.WindowStaysOnTopHint | Qt.WindowType.Tool | Qt.WindowType.WindowTransparentForInput
        ...
        self.primary_label = QLabel(); self.secondary_label = QLabel()
        for label in (self.primary_label, self.secondary_label):
            label.setWordWrap(True); label.setTextFormat(Qt.TextFormat.PlainText)
            label.setAlignment(Qt.AlignmentFlag.AlignHCenter | Qt.AlignmentFlag.AlignVCenter)
        layout = QVBoxLayout(self.frame); layout.setContentsMargins(18, 8, 18, 8); layout.setSpacing(2)
        layout.addWidget(self.primary_label); layout.addWidget(self.secondary_label)
        outer = QVBoxLayout(self); outer.setContentsMargins(0, 0, 0, 0); outer.addWidget(self.frame)
    ...
    def apply_config(self, config: dict[str, Any]) -> None:
        ...
        self.frame.setStyleSheet("QFrame#subtitleFrame { background-color: rgba(0, 0, 0, 160); border-radius: 7px; }")
    ...
    def _relayout(self) -> None:
        ...
        secondary_height = self.secondary_label.fontMetrics().boundingRect(
            QRect(0, 0, content_width, 10000), Qt.TextFlag.TextWordWrap, self.secondary_label.text()
        ).height()
        primary_height = 0
        if self.config["display_mode"] == "two_lines":
            primary_height = self.primary_label.fontMetrics().boundingRect(
                QRect(0, 0, content_width, 10000), Qt.TextFlag.TextWordWrap, self.primary_label.text()
            ).height()
        height = max(font_size + 30, primary_height + secondary_height + 30)
        left = x + (game_width - width) // 2
        bottom = y + game_height - int(game_height * int(self.config["bottom_offset_percent"]) / 100)
...
def create_ui(callbacks: dict[str, Any] | None = None, config: dict[str, Any] | None = None,
              languages: list[Any] | None = None) -> tuple[SettingsWindow, OverlayWindow]:
