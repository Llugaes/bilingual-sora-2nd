...
from locales import archive_names
        ...
        for language, group in groups.items():
            if len(group)>1 and any(value!=group[0] for value in group):
                audit["ambiguous_duplicate_groups"].append({
                    "path": path, "class": kind, "fingerprint": stable_id, "language":language,"records_by_language": sizes,
                })
                audit["counters"]["duplicate_groups_rejected"] += 1
                continue
            if len(group)>1:audit["counters"]["duplicate_groups_coalesced"] += 1
            if group:
...
def build_table_entries(game_dir):
    from menu_tables import build_menu_entries
