...
def visual_secondary(text):
    ...
    return re.sub(r'<[^<>]*>', lambda m: m[0] if re.fullmatch(
        r'<R>|</R[^<>]*>|</?[Cc][0-9a-fA-F]*>|</?B>|<[sS]\d+>|<I\d+>', m[0]) else '', text)
...
def annotation_plan(primary, secondary):
    ...
    if left_count!=len(right) or any(bool(a.strip())!=bool(b.strip()) for a,b in zip(lines[::2],right)):
    ...
    for i,part in enumerate(lines):
        if i%2:out+=part;continue
        payload=right[i//2]
        if payload.strip():
            layers.append({'offset':len(out.encode('utf-8'))+6,'text':payload,
...
def ruby(primary, secondary):
    if not primary or not secondary:return primary
    identical_cjk=primary==secondary and bool(re.search(r'[\u3040-\u30ff\u3400-\u9fff\uac00-\ud7af]',primary))
    if primary==secondary and not identical_cjk:return primary
    # A ruby body cannot contain nested tags. Controls live outside its runs.
    if '<' in primary+secondary or '>' in primary+secondary: return None
    p=re.split(r'(\r\n|\n)',primary.replace('\\n','\n'))
    s=re.split(r'\r\n|\n',secondary.replace('\\n','\n'))
    if (len(p)+1)//2!=len(s):return None  # needs independent annotation lanes
    out=[]
    for i,a in enumerate(p):
        if i%2:out.append(a);continue
        b=s[i//2]
        if bool(a.strip())!=bool(b.strip()):return None
...
def overdrive_descriptions(entries):
    ...
    """
    template=next((e['texts'] for e in entries if e.get('key')==
        'table/t_text.tbl/TXT_CAMP_STATUS_OVERDRIVE_INFO_TEMPLATE'),None)
    if not template:return []
    rows={}
    for e in entries:
        if e.get('key','').startswith('table/t_condition_info.tbl/OverDriveEffect/'):
            record,field=e['key'].rsplit('/',1);rows.setdefault(record,{})[field]=e['texts']
    result=[]
    ...
    for key,fields in rows.items():
        ...
        for lang in fields.get('name',{}):
            ...
            if pattern.count('%s')!=6 or '%' in pattern.replace('%s',''):continue
        ...
        if texts:
            ...
            result.append({'key':key+'/assembled_description_trimmed','texts':{k:v.rstrip() for k,v in texts.items()}})
    return result


class MenuTranslator:
    def __init__(self,entries,primary,secondary,source_language=DEFAULT_PRIMARY,_details_only=False):
        ...
        if not _details_only:
            ...
            entries += [{'key':e['key']+'/formatted','texts':{l:t+'%d' for l,t in e['texts'].items()}}
                for e in entries if e.get('key')=='table/t_text.tbl/TXT_SAVE_DETAIL_LEVEL']
        self.same_language=primary==secondary
        self.scoped={}
        self.detail_sources=set()
        self.details=None
        if not _details_only:
            for scope,prefix in [('support','table/t_support_ability.tbl/'),
                                 ('overdrive','table/t_condition_info.tbl/OverDriveEffect/')]:
                selected=[e for e in entries if e.get('key','').startswith(prefix)]
                if selected:self.scoped[scope]=MenuTranslator(selected,primary,secondary,source_language,True)
            detail_entries=[e for e in entries if e.get('key','').startswith(
                ('table/t_item.tbl/','table/t_itemhelp.tbl/','table/t_skill.tbl/','table/t_support_ability.tbl/'))]
            self.detail_sources={e['texts'][source_language] for e in detail_entries
                if e.get('key','').endswith('/description') and source_language in e['texts']}
            if detail_entries:
                self.details=MenuTranslator(detail_entries,primary,secondary,source_language,True)
        self.keyed=[]
        candidates={}
        for entry in entries:
            ...
            for value in {texts[source_language]} if source_language in texts else set():
                for source in {value,plain(value)}:
                    if source.strip(): candidates.setdefault(source,set()).add(pair)
                ...
                if visible!=value and visible.strip():
                    candidates.setdefault(visible,set()).add(tuple(display_text(t) for t in pair) if pair else None)
        ...
        names={}
        for entry in entries:
            key=entry.get('key','');texts=entry['texts']
            if key.startswith(('table/t_name.tbl/','table/t_status.tbl/')) and key.endswith('/name') and source_language in texts:
                pair=complete_pair(texts,primary,secondary)
                names.setdefault(texts[source_language],set()).add(pair)
        for source,pairs in names.items():
            if len(pairs)==1 and None not in pairs:candidates[source]=pairs
        self.pairs={s:next(iter(p)) for s,p in candidates.items() if len(p)==1 and None not in p}
        ...
        for source,pairs in candidates.items():
            ...
            if len(p)==1 and None not in p: normalized[source]=next(iter(p))
        self.plain_pairs=normalized
        self.numeric=[];self.raw_numeric=[]
        for source,pair,is_raw in [(s,p,False) for s,p in normalized.items()]+[(s,p,True) for s,p in self.pairs.items() if '<' in s]:
            matches=list(FORMAT.finditer(source))
            if not matches or len(matches)>4 or any('%' in FORMAT.sub('',t) for t in (source,*pair)):continue
            # Number arguments retain order unless explicit positional slots are
            # available; reject mismatched placeholder contracts.
            if any(len(FORMAT.findall(t))!=len(matches) for t in pair):continue
            if any('$' in m.group() for t in (source,*pair) for m in FORMAT.finditer(t)):continue
            kinds=[m.group()[-1] for m in matches]
            if any([m.group()[-1] for m in FORMAT.finditer(t)]!=kinds for t in pair):continue
            if 's' in kinds and len(FORMAT.sub('',source).strip())<2:continue
            chunks=[];at=0
            for m in matches:
                chunks.extend((re.escape(source[at:m.start()]),r'([^<>\r\n]{1,512}?)' if m.group()[-1]=='s' else r'([+-]?\d+)'))
                at=m.end()
            chunks.append(re.escape(source[at:]))
            (self.raw_numeric if is_raw else self.numeric).append((re.compile(''.join(chunks)),pair))

    def raw_pair(self,source):
        if source in self.pairs:return self.pairs[source]
    ...
    def pair(self,source):
        if source in self.plain_pairs:return self.plain_pairs[source]
        matches=set()
    ...
    def component(self,source,mode):
        ...
        if len(parts)>1:
            return ''.join(self.component(p,mode) if i%2==0 else p for i,p in enumerate(parts))
        return source

    def translate(self,source,mode='annotation'):
        if self.same_language and mode=='annotation':mode='primary'
        ...
        if mode=='bilingual':
            a=self.translate(source,'primary');b=self.translate(source,'secondary')
        ...
        return ''.join(t if i%2 else self.component(t,mode) for i,t in enumerate(TOKEN.split(source)))
    ...
    def render(self,source,mode='annotation'):
        ...
        if mode=='annotation' and known:
            ...
            if prefix and '<' not in body+visible_b and len(re.split(r'\r\n|\n|\\n',body))==len(re.split(r'\r\n|\n|\\n',visible_b)):
                value=ruby(body,visible_b)
                if value is not None:
                    text=prefix+value
                    return {'text':text,'layers':[],'kind':'ruby' if text!=source else 'plain'}
        left=re.split(r'\r\n|\n|\\n',a);right=re.split(r'\r\n|\n|\\n',b)
        different_lines=len(left)!=len(right) or any(bool(x.strip())!=bool(y.strip()) for x,y in zip(left,right))
        if mode=='annotation' and ((known and (any(c in a+b for c in '<>') or different_lines)) or
                (a!=b and ('<R>' in a+b or different_lines))):
            # Original tags remain byte-for-byte in the primary text. A
            # separate lane avoids nested ruby and preserves native notation.
            if visual_secondary(b).strip():return annotation_plan(a,b)
        text=self.translate(source,mode)
        return {'text':text,'layers':[], 'kind': 'ruby' if mode=='annotation' and
                text!=source and '<R>' in text else 'plain'}

    def dictionary(self,mode):
        if self.same_language and mode=='annotation':mode='primary'
        result={s:t for s in self.pairs if (t:=self.translate(s,mode))!=s}
        for scope,tr in self.scoped.items():
            result.update({'\x02'+scope+'\x00'+s:t for s in tr.pairs if (t:=tr.translate(s,mode))!=s})
        for key,source,(a,b) in self.keyed:
            one=MenuTranslator([{'texts':{'source':source,'primary':a,'secondary':b}}],
                               'primary','secondary','source')
            text=one.translate(source,mode)
            if text!=source:result['\x01'+key+'\x00'+source]=text
        return result
    ...
    def runtime_model(self):
        ...
        for key,source,(a,b) in self.keyed:
            one=MenuTranslator([{'texts':{'source':source,'primary':a,'secondary':b}}],
                               'primary','secondary','source',True)
            keyed[key]={'source':source,'model':one.runtime_model()}
        return {'pairs':self.pairs,'plain_pairs':self.plain_pairs,
                ...
                'same_language':self.same_language,'keyed':keyed,
