...
SCHEMAS = {
    ...
    "ConditionHelpData": spec(
        40, [("format", 8), ("condition", 16), ("rate", 24), ("next_condition", 32)]
    ),
    "SkillEffectTypeHelpData": spec(24, [("label", 8), ("suffix", 16)]),
    "SkillEffectHelpData": spec(
        88, [("name", 8), ("stat", 32), ("format", 40), ("turns", 72), ("value", 80)], [16, 48, 56]
    ),
    ...
    "AchievementTableData": spec(
        96, [("title", 72), ("description", 80), ("counter", 88)], [24, 64]
    ),
...
def sections(data):
    ...
    for i in range(count):
        ...
        try:
            ...
            raise FormatError("non-ASCII section name") from exc
...
def schema_for(path, kind):
    if path == "table/t_quest_fc.tbl":
        if kind == "QuestText":
            return spec(32, [("body", 8), ("detail", 24)])
        if kind == "NoteMainHistory":
            return spec(16, [("body", 0)])
    return SCHEMAS[kind]
    ...
    for i in range(count):
        ...
        for name, offset in schema.fields:
            ...
            if not pointer:
                continue
            if not text_floor <= pointer < len(data):
                raise FormatError("text pointer outside string pool: " + kind)
            end = data.find(b"\0", pointer)
            if end < 0:
                raise FormatError("unterminated string: " + kind)
            try:
                value = data[pointer:end].decode("utf-8")
            except UnicodeDecodeError as exc:
                raise FormatError("invalid UTF-8: " + kind) from exc
    ...
    try:
        for language, filename in _TABLE_ARCHIVES.items():
            ...
            if archive_path.exists():
                archives[language] = FpacArchive(archive_path)
            else:
                audit["diagnostics"].append(
                    {"language": language, "reason": "missing table archive"}
                )
        if not archives:
            raise FileNotFoundError("No table archives found")
        ...
        for path in sorted(set().union(*(p.keys() for p in paths.values()))):
            ...
            for l, data in files.items():
                try:
                    headers[l] = sections(data)
                except FormatError as exc:
                    audit["diagnostics"].append({"path": path, "language": l, "reason": str(exc)})
            if not headers:
                audit["unsupported_tables"].append(
                    {"path": path, "reason": "invalid section descriptors"}
                )
                continue
            section_types = {}
            ...
            for (kind, occurrence), (index, section) in section_types.items():
                ...
                if kind != "TextTableData" and kind not in SCHEMAS:
                    audit["unrecognized_tables"].append({"path": path, "class": kind})
                    continue
                ...
                for l, data in files.items():
                    ...
                    try:
                        ...
                        audit["diagnostics"].append(
                            {"path": path, "class": kind, "language": l, "reason": str(exc)}
                        )
                if not parsed:
                    audit["unsupported_tables"].append({"path": path, "class": kind})
                    continue
                key_path = (
                    path
                    if index == 0
                ...
                audit["coverage"].append(
                    {
                        "path": key_path,
                        "class": kind,
                        "records_by_language": {l: len(r) for l, r in parsed.items()},
                        "entries_emitted": len(entries) - before,
                    }
                )
