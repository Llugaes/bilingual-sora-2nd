...
from sora_bilingual.updates.update_installer import install, UpdateBusy, write_json
    ...
    def _run(self, manual):
        ...
        with self._lock:
            try:
                ...
                self.message = "更新未完成：" + str(exc)
                self.next_check = self.clock() + 900
                write_json(
                    self.directory / "last-error.json", {"time": self.clock(), "error": str(exc)}
                )
            finally:
                self.progress = None
