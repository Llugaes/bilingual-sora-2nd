...
from sora_bilingual.fonts.font_merge import parse_fnt, parse_dds
from sora_bilingual.fonts.universal_fonts import MAX_DECODED_BYTES
from sora_bilingual.config.native_config import write_config, replace_file
import tempfile
import os
...
def main():
    ...
    parser.add_argument(
        "--update", action="store_true", help="replace only files matching our previous receipt"
    )
    ...
    if sha(loader) != LOADER_SHA:
        raise RuntimeError("Loader digest mismatch")
    expected = {
        f"asset{v.font_suffix}/{tail}"
        for v in LOCALES.values()
    ...
    for item in manifest["files"]:
        ...
        if not path.is_relative_to(candidate.resolve()):
            raise RuntimeError("Candidate escaped root")
    ...
    for prefix in {f"asset{v.font_suffix}" for v in LOCALES.values()}:
        ...
        if len(dds) > MAX_DECODED_BYTES:
            raise RuntimeError("Font exceeds game buffer")
        image = parse_dds(dds)
        parse_fnt(
    ...
    receipt = {
        ...
        "game": str(game),
        "files": [
            {"path": str(path), "size": len(data), "sha256": sha(data)}
    ...
    if args.install:
        ...
        if any(
            p.name.lower() == "sora_2nd.exe" for p in frida.get_local_device().enumerate_processes()
        ):
