...
def parse_fnt(data: bytes, *, atlas_width: int = 4096, atlas_height: int = 4096) -> FntFont:
    ...
    if len(data) != expected_size:
        raise FontFormatError(
            f"FNT count {count} requires {expected_size} bytes, got {len(data)}"
        )
...
def build_manifest(primary_fnt: bytes, secondary_fnt: bytes, primary_dds: bytes, secondary_dds: bytes,
                   merged_fnt: bytes, merged_dds: bytes) -> dict[str, object]:
    ...
    return {
        ...
        "sha256": {
            "primary_fnt": _sha256(primary_fnt), "secondary_fnt": _sha256(secondary_fnt),
            "primary_dds": _sha256(primary_dds), "secondary_dds": _sha256(secondary_dds),
            "merged_fnt": _sha256(merged_fnt), "merged_dds": _sha256(merged_dds),
        },
    }


def write_merge(output_dir: Path, primary_fnt: bytes, secondary_fnt: bytes,
                primary_dds: bytes, secondary_dds: bytes) -> dict[str, object]:
    ...
    manifest = build_manifest(primary_fnt, secondary_fnt, primary_dds, secondary_dds, merged_fnt, merged_dds)
...
def extract_game_sources(game_dir: Path) -> tuple[bytes, bytes, bytes, bytes]:
    ...
    try:
        ...
        raise RuntimeError("game extraction needs lz4.frame; use direct DDS paths otherwise") from exc
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Create an unpacked SC-primary + JP font candidate")
    parser.add_argument("--output", required=True, type=Path, help="explicit output directory (never game directory)")
    parser.add_argument("--game", type=Path, help="explicit game directory to read with FpacArchive")
