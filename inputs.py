...
KEY_TO_VK = {
    ...
    "WIN": 0x5B, "SPACE": 0x20, "TAB": 0x09, "ENTER": 0x0D,
...
class InputManager:
            ...
            try:
                ...
                for hat in range(stick.get_numhats()):
                    ...
                    for direction,pressed in enumerate((y>0,x>0,y<0,x<0)):
                        if pressed:buttons.add(1000+hat*4+direction)
    ...
    def _matching_gamepads(self) -> list[GamepadState]:
        ...
        return [
            ...
            and all(0<=a['index']<len(device.axes) and
                    (device.axes[a['index']]-a['threshold'])*a['direction']>0 for a in axes)
    ...
    def poll_capture(self) -> dict[str, Any] | None:
        ...
        for device in self.devices():
            ...
            excursions={i:{'index':i,'direction':1 if v>n else -1,'threshold':(v+n)/2}
                        for i,(v,n) in enumerate(zip(device.axes,neutral)) if abs(v-n)>.7}
            ...
            if self._capture_device and device.instance_id == self._capture_device.instance_id:
                still_axes=any(a['index']<len(device.axes) and
                    (device.axes[a['index']]-a['threshold'])*a['direction']>0 for a in self._capture_axes.values())
                if device.buttons or excursions or still_axes:
                    self._capture_buttons.update(device.buttons)
                    for i,a in excursions.items():self._capture_axes.setdefault(i,a)
                    return None
                self._capturing_controller = False
                self.capture_status = "complete"
                binding={'guid':self._capture_device.guid,'buttons':sorted(self._capture_buttons)}
                if self._capture_axes:binding['axes']=list(self._capture_axes.values())
