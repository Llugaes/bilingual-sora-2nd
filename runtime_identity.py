...
from resources import FpacArchive, _ARCHIVES, _logical_script_entries, _utf8z, _parse_code
from menu_text import MenuTranslator,complete_pair
...
def script_signature(data):
    ...
    return (data[:24] + data[start:start+32] + data[start+(count-1)*32:start+count*32]).hex()
...
def compile_script_identities(game, entries, primary, secondary, language):
    ...
    ambiguous = {s for s, pairs in candidates.items() if len(pairs) > 1 and any(
        pair and all(re.sub(r'<[^<>]*>', '', t).strip() for t in pair) and
        (pair[0] != pair[1] or re.search(r'[\u3040-\u30ff\u3400-\u9fff\uac00-\ud7af]', pair[0]))
        for pair in pairs)}
    ...
    result = {}; stats = defaultdict(int); pointers=defaultdict(list); pointer_models={};blocked_functions=set()
    ...
    try:
        ...
        for path in sorted(paths):
            ...
            start, count = struct.unpack_from('<II', data, 4)
            names=tuple(_utf8z(data,struct.unpack_from('<I',data,start+n*32+28)[0]&0x3fffffff) for n in range(count))
            global_at,global_count=struct.unpack_from('<II',data,12)
            globals_=tuple(_utf8z(data,struct.unpack_from('<I',data,global_at+n*8)[0]&0x3fffffff) for n in range(global_count))
            code_starts=sorted(struct.unpack_from('<I',data,start+n*32)[0] for n in range(count))
            ...
            for number in range(count):
                ...
                for entry in selected:
                    ...
                    if source not in ambiguous or not complete_pair(entry['texts'],primary,secondary):continue
                    suffix=entry['key'].split('/alignment/',1)[0].split('.dat/',1)[1].split('/')[1:];offset=None
                    if len(suffix)==4 and suffix[0]=='called' and suffix[2]=='arg':
                        ...
                        if slot<argc:
                            ...
                            if k==0 and v>>30==3:offset=v&0x3fffffff
                    elif len(suffix)==2 and suffix[0]=='code':
                        if code_offsets is None:
                            code_offsets=[];code_at=struct.unpack_from('<I',data,at)[0]
                            next_at=next((v for v in code_starts if v>code_at),None)
                            _parse_code(data,code_at,next_at,number,names,globals_,code_offsets)
                        offset=code_offsets[int(suffix[1])]
                    if offset is not None:
                        if _utf8z(data,offset)!=source:raise ValueError('Script pointer/catalog disagreement')
                        pointer_models[entry['key']]={'source':source,'model':MenuTranslator([entry],primary,secondary,language,True).runtime_model()}
                        pointers[source].append({'key':entry['key'],'offset':offset,'size':len(data),'sha256':digest,'header':data[:24].hex()})
                local = MenuTranslator(selected, primary, secondary, language, True)
                call_count, call_start = struct.unpack_from('<II', data, at+16)
                by_call = defaultdict(list)
                for entry in selected:
                    if language not in entry['texts']:continue
                ...
                for call, values in by_call.items():
                    ...
                    _, kind, argc, args_at = struct.unpack_from('<IHHI', data, call_start+call*12)
                    if kind != 3 or argc < 3 or args_at+argc*8 > len(data):
                        continue
                    args = [struct.unpack_from('<II', data, args_at+i*8) for i in range(argc)]
                    if any(k != 0 for _, k in args):stats['dynamic_calls'] += 1
                    if args[0][0] != 0x40000005 or args[1][0] not in (0x40000000,0x40000006,0x40000007,0x40000013):
                        continue
                ...
                for token, values in groups.items():
                    ...
                    for entry in values:
                        t = entry['texts']; s = t.get(language)
                ...
                if function_identity in blocked_functions:continue
        ...
        for source,items in pointers.items():
            for item in items:
                address_pairs[item['sha256'],item['offset']].add(tuple(pointer_models[item['key']]['model']['pairs'][source]))
        blocked={address for address,pairs in address_pairs.items() if len(pairs)>1}
        stats['shared_script_pointer_conflicts']=len(blocked)
        pointers={source:[item for item in items if (item['sha256'],item['offset']) not in blocked] for source,items in pointers.items()}
        ...
        return {'scripts': result, 'stats': dict(stats),'pointers':dict(pointers),'pointer_models':pointer_models}
    finally:
        archive.close()
...
def compile_table_identities(game, entries, primary, secondary, language):
    ...
    from menu_tables import sections, schema_for, SCHEMAS
    global_tr = MenuTranslator(entries, primary, secondary, language)
    needed = {e['key']: e for e in entries if e.get('key', '').startswith('table/')
              and e['texts'].get(language,'').strip() and complete_pair(e['texts'],primary,secondary)
              and e['texts'][language] not in global_tr.pairs}
    sources = defaultdict(list); models = {}; files = {}
    ...
    try:
        ...
        for path, actual in logical.items():
            if not any(k.startswith(path+'/') for k in needed):continue
            data = archive.read(actual); headers = sections(data)
            floor = max(start+stride*count for _,start,stride,count in headers)
            file_id = hashlib.sha256(data).hexdigest()
            files[file_id] = {'header': data[:8+80*len(headers)].hex(),
                              'floor': floor, 'size': len(data),
                              'pool_sha256': hashlib.sha256(data[floor:]).hexdigest()}
            for index, (kind,start,stride,count) in enumerate(headers):
                if kind not in SCHEMAS:continue  # t_text already has native hash keys
                schema = schema_for(path,kind)
                if stride != schema.size:continue
                occurrence = sum(s[0]==kind for s in headers[:index])
                prefix = path if index==0 else path+'/'+kind+(f'/{occurrence}' if occurrence else '')
                for number in range(count):
                    at=start+number*stride; record=bytearray(data[at:at+stride])
                    for offset in schema.pointers:record[offset:offset+8]=b'\0'*8
                    from menu_tables import record_identity
                    ...
                    for field,offset in schema.fields:
                        ...
                        entry=needed[key];source=entry['texts'][language]
                        pointer=struct.unpack_from('<Q',data,at+offset)[0]
                        if not floor<=pointer<len(data) or _utf8z(data,pointer)!=source:continue
                        one=MenuTranslator([entry],primary,secondary,language,True).runtime_model()
                        models[key]={'source':source,'model':one}
                        sources[source].append({'key':key,'file':file_id,'offset':pointer,
                            'record_at':at,'field_at':offset,'record':record.hex(),'pointers':schema.pointers})
